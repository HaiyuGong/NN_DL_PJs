...
writer = SummaryWriter(log_dir=f'runs6/resnet18_pretrained{pretrained}_lr{lr}_epoch{num_epochs}_dropout{dropout}_{optm}_lrsch{lrsch}_{suffix_num}')
...
for epoch in range(num_epochs):
    ...
    if test_accuracy > best_acc:
        ...
        if (pretrained and best_acc > 0.76) or  (not pretrained and best_acc > 0.5):
            torch.save(model.state_dict(), f'saved_models3/resnet18_pretrained{pretrained}_lr{lr}_epoch{num_epochs}_dropout{dropout}_{optm}_acc{best_acc:.6f}.pth')
