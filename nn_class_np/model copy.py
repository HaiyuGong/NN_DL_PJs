...
def train(w1, w2, X_train, y_train, X_test, y_test, lambda_, learning_rate, num_epochs, batch_size):
    ...
    for epoch in range(num_epochs):
        ...
        for i in range(num_batches):
            ...
            x = X_train_permuted[start:end]
            y = y_train_permuted[start:end]
            ...
            train_acc_list.append(accuracy(y, y_pred))
        ...
        acc_train = sum(train_acc_list) / len(train_acc_list)
        h_test = 1 / (1 + np.exp(-X_test.dot(w1)))
        y_test_pred = h_test.dot(w2)
...
N, D_in, H, D_out = 128, 784, 100, 10
# x, y = randn(N, D_in), randn(N, D_out)
w1, w2 = randn(D_in, H), randn(H, D_out)


train(w1,w2, X_train, y_train_onehot, X_test, y_test_onehot, lambda_=0.0001, learning_rate=0.1, num_epochs=100, batch_size=128)